...
func _process(delta: float) -> void:
	...
	# Smooth lateral follow, instant forward follow
	...
	new_pos.z = target_pos.z  # lock to player Z so it never lags behind
	global_position = new_pos

	# Always look slightly ahead and down
	look_at(player.global_position + look_offset, Vector3.UP)
